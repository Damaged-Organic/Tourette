...
const DICTIONARY_MAX_LENGTH = 200;
...
const Tourette = {
    ...
    convert() {
        ...
        if( editor ) {
            ...
            if( selection ) {
                let reducedDictionary = this.dictionary.reduce((reduced, initial) => {
                ...
                let convertedSelection = selection.replace(/./g, (character) => {
                    ...
                    if( reducedDictionary[charCodeAt] && reducedDictionary[charCodeAt].shift ) {
                        qwertyCharacter = this.qwertyMatrix.shift[reducedDictionary[charCodeAt].shift];
                    } else {
                        qwertyCharacter = this.qwertyMatrix[reducedDictionary[charCodeAt]];
                    }
